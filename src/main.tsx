...
import { sceneWorldRayToRuntime } from './runtime/runtime-coordinates'
...
type NormalizeStoredProjectOptions = {
  ...
  /** Keep legacy asset instances for a caller that needs to remap live IDs. */
  materializeInstances?: boolean
}

type MaterializeProjectInstancesResult = {
  project: ProjectState
  /** Old scene-part IDs to the custom scene-part IDs created by materialization. */
 ...
 */
function materializeProjectInstancesWithMap(project: ProjectState): MaterializeProjectInstancesResult {
  if (!project.instances.length) return { project, idMap: new Map() }
  ...
  project.instances.forEach((instance) => {
    ...
    const resolvedPartIds = [...new Set(resolvedPairs.map(({ partId }) => partId))]
    ...
    componentPairs.forEach((pairs, partId) => {
      ...
      // sceneEntityParts() exposes asset instances as asset:<instance>:<part>
      // while the materialized scene exposes the same editable part as
      // custom:<entity>. Keep this mapping explicit so live selection/edit
      // state never continues to point at the obsolete instance ID.
      // A non-assembly asset may still contain several source component
      // arrays. `sceneEntityParts()` exposes the original component id in
      // the live asset-instance id, while materialization intentionally
      // collapses those arrays into one ordinary custom entity. Map every
      // original alias, not only the synthetic `__asset__` bucket, otherwise
      // a selected imported/model-converted part can keep pointing at the
      // obsolete instance coordinate path during the first edit.
      const sourcePartAliases = asset.assembly
        ? [partId]
        : [...new Set([partId, ...resolvedPartIds, '__asset__'])]
      sourcePartAliases.forEach((alias) => {
        idMap.set(`asset:${instance.id}:${alias}`, `custom:${entityId}`)
        partKeyMap.set(alias, [entityId])
      })
      if (!asset.assembly) idMap.set(`asset:${instance.id}`, `custom:${entityId}`)
      ...
      const metadataAlias = sourcePartAliases.find((alias) => project.entityNames?.[`asset:${instance.id}:${alias}`] !== undefined)
        ?? partId
      copyNameMetadata(`asset:${instance.id}:${metadataAlias}`, `voxel:${entityId}`)
    ...
    if (asset.assembly) {
      ...
      // A selected asset assembly is represented in the scene tree by an
      // assembly:<id> target. Keep that target valid after materialization;
      // the assembly itself remains a scene assembly, so its id is stable.
      const rootAssemblyIds = (asset.assembly.rootIds?.length
        ? asset.assembly.rootIds
        : [asset.assembly.rootId])
        .map((rootId) => assemblyMap.get(rootId))
        .filter((rootId): rootId is string => Boolean(rootId))
      if (rootAssemblyIds.length === 1) {
        idMap.set(`asset:${instance.id}`, `assembly:${rootAssemblyIds[0]}`)
      }
    }
  })
...
function materializeProjectInstances(project: ProjectState): ProjectState {
  return materializeProjectInstancesWithMap(project).project
}
...
function normalizeStoredProject(loaded: ProjectState, options: NormalizeStoredProjectOptions = {}): ProjectState {
  ...
  if (options.materializeInstances !== false) {
...
/**
 * Make a history-owned snapshot. Scene collections and voxel records are
 * copied so later batched edits cannot mutate a past/future entry. Large
 * catalog geometry remains shared because editor operations replace asset
 * records instead of mutating their voxel arrays in place.
 */
function cloneProjectForHistory(source: ProjectState): ProjectState {
  const snapshot = cloneProjectForMutation(source)
  snapshot.customVoxels = source.customVoxels.map((voxel) => ({ ...voxel }))
  snapshot.assets = source.assets.map((asset) => ({
    ...asset,
    categoryPath: asset.categoryPath ? [...asset.categoryPath] : undefined,
    parts: [...asset.parts],
    assembly: asset.assembly ? {
      ...asset.assembly,
      rootIds: asset.assembly.rootIds ? [...asset.assembly.rootIds] : undefined,
      partNames: asset.assembly.partNames ? { ...asset.assembly.partNames } : undefined,
      nodes: asset.assembly.nodes.map((node) => ({ ...node, memberKeys: [...node.memberKeys] })),
    } : undefined,
  }))
  return snapshot
}
...
function sameHistoryProject(left: ProjectState, right: ProjectState): boolean {
  try {
    // Project roots and voxel arrays may be reused by the fast editor paths.
    // Always inspect current content; object identity is not a correctness
    // signal for a mutable project.
    return sceneContentSignature(left) === sceneContentSignature(right)
  } catch {
...
type VoxelStrokeTransaction = {
  ...
  occupiedSceneKeys: Set<string>
  /**
   * Exact scene-space ownership captured at pointer-down. The occupancy index
   * is deliberately incremental and can still contain the previous React
   * snapshot for one frame after a model/template is materialized. Editing
   * must never treat that window as empty space.
   */
  ownerIdsBySceneKey: Map<string, Set<string>>
...
function App() {
  ...
  const checkedTreePartIdsRef = useRef<string[]>([])
  ...
  // The edit target can be remapped when a legacy asset instance is
  // materialized at stroke start. Keep the mapped id outside React state so
  // pointer events in the same gesture never read the obsolete asset:* id.
  const voxelStrokeEditEntityRef = useRef<string | null>(null)
  ...
  const makeHistoryEntry = (
    historyProject: ProjectState,
    metadata: Partial<Omit<ProjectHistoryEntry, 'project'>> = {},
  ): ProjectHistoryEntry => ({
    project: cloneProjectForHistory(historyProject),
    editEntityId: metadata.editEntityId !== undefined ? metadata.editEntityId : editEntityIdRef.current,
    selectedId: metadata.selectedId !== undefined ? metadata.selectedId : selectedIdRef.current,
    checkedTreePartIds: metadata.checkedTreePartIds ? [...metadata.checkedTreePartIds] : [...checkedTreePartIdsRef.current],
  })
  ...
  const recordHistoryBeforeChange = (
    ...
    const afterEntry: ProjectHistoryEntry = {
      ...makeHistoryEntry(after),
    }
    const recorded = recordHistoryTransition(
      historyRef.current,
      makeHistoryEntry(before),
      afterEntry,
      // Content equality was checked before snapshotting. Never compare the
      // snapshots again by a stale identity/cache while recording them.
      () => false,
  ...
  const beginVoxelStroke = () => {
    ...
    // A scene instance is only a legacy persistence representation. Before a
    // stroke starts, materialize it once so the transaction has exactly the
    // same scene-space custom-voxel representation as a hand-drawn entity.
    const originalWithInstances = projectRef.current.instances.length
      ? normalizeStoredProject(projectRef.current, { normalizeNaming: false, materializeInstances: false })
      : projectRef.current
    const materialized = originalWithInstances.instances.length
      ? materializeProjectInstancesWithMap(originalWithInstances)
      : { project: originalWithInstances, idMap: new Map<string, string>() }
    const original = materialized.project
    const remapLiveId = (id: string | null | undefined) => id
      ? (materialized.idMap.get(id) ?? id)
      : id ?? null
    const mappedEditEntityId = remapLiveId(editEntityId)
    const mappedSelectedId = remapLiveId(selectedId) ?? ''
    const mappedCheckedTreePartIds = checkedTreePartIds.map((id) => remapLiveId(id) ?? id)
    voxelStrokeEditEntityRef.current = mappedEditEntityId
    if (original !== projectRef.current) {
      projectRef.current = original
      sceneOccupancyRef.current?.syncParts(sceneEntityParts(original))
      skipSceneOccupancySyncRef.current = true
      editEntityIdRef.current = mappedEditEntityId
      selectedIdRef.current = mappedSelectedId
      checkedTreePartIdsRef.current = mappedCheckedTreePartIds
      setProject(original)
      setEditEntityId(mappedEditEntityId)
      setSelectedId(mappedSelectedId)
      setCheckedTreePartIds(mappedCheckedTreePartIds)
    }
    const draft = cloneProjectForMutation(original, { shareCatalogs: true })
    const initialParts = sceneEntityParts(original)
    const occupiedSceneKeys = new Set<string>()
    const ownerIdsBySceneKey = new Map<string, Set<string>>()
    // Occupancy is a scene concern, not a storage-kind concern. A legacy
    // instance can still be present for one render/update cycle before it is
    // materialized, and it must block an edit exactly like a hand-drawn part.
    // Filtering to `custom` here made imported models appear hollow to the
    // brush and allowed a new voxel to overwrite an existing model cell.
    initialParts.forEach((part) => {
      scenePartVoxels(part).forEach((voxel) => {
        const key = sceneVoxelKey(voxel)
        occupiedSceneKeys.add(key)
        const owners = ownerIdsBySceneKey.get(key) ?? new Set<string>()
        owners.add(part.id)
        ownerIdsBySceneKey.set(key, owners)
      })
    })
    voxelStrokeTransactionRef.current = {
      draft,
      original,
      historyEditEntityId: mappedEditEntityId,
      historySelectedId: mappedSelectedId,
      historyCheckedTreePartIds: [...mappedCheckedTreePartIds],
  ...
  const commitVoxelStroke = () => {
    ...
    voxelStrokeEditEntityRef.current = null
    if (!transaction) return
    ...
    if (!sameHistoryProject(transaction.original, next)) {
  ...
  const commitProject = (next: ProjectState, trackHistory = true) => {
    if (geometryApplyingRef.current) return
    if (!trackHistory) {
      // Loading/new-scene replacement starts a new history domain. Keeping
      // entries from the previous scene makes undo/redo restore unrelated
      // project roots and is the source of several delayed "reappearing"
      // entities after a later edit.
  ...
  const commitGeometryProject = async (next: ProjectState, removedPartIds: string[], resultGroups: Array<{ entityId: string; voxels: Voxel[] }>) => {
    ...
    // history snapshots are cloned at this boundary, so the previous root can
    // remain shared with the live read-only state.
  ...
  const restoreHistoryEntry = (entry: ProjectHistoryEntry, notice: string) => {
    ...
    // Do not make a history entry's own object the live mutable project. A
    // later fast edit may structurally share nested catalog data, and an
    // in-place normalizer or renderer refresh must never be able to mutate a
    // snapshot that is still needed by redo.
    const restoredProject = cloneProjectForHistory(entry.project)
    const targetParts = sceneEntityParts(restoredProject)
    // Undo/redo is a state-domain switch, not an ordinary owner update. A
    // partial owner sync can leave stale cells from an erased entity (or stale
    // transformed coordinates) in the occupancy index; those cells then make
    // later drawing, picking and collision checks disagree with the restored
    // React project. Rebuild the complete index atomically at this boundary.
    sceneOccupancyRef.current?.syncParts(targetParts)
    skipSceneOccupancySyncRef.current = true
    // Invalidate worker callbacks and deferred React transitions that were
    // created before this restore. The epoch is advanced here as well as in
    // cancelPendingHistoryInteractions because restoreHistoryEntry is the
    // single authoritative publication point for both undo and redo.
    historyEpochRef.current += 1
    projectRef.current = restoredProject
    selectedIdRef.current = entry.selectedId
    editEntityIdRef.current = entry.editEntityId
    checkedTreePartIdsRef.current = [...entry.checkedTreePartIds]
    markSceneDirty()
  ...
  const addVoxel = (voxel: Voxel) => {
    ...
    const activeEditEntityId = voxelStrokeEditEntityRef.current
      ?? editEntityId
      ?? (voxelStrokeEntityRef.current ? `custom:${voxelStrokeEntityRef.current}` : null)
    const editAssemblyId = activeEditEntityId?.startsWith('assembly:') ? activeEditEntityId.slice('assembly:'.length) : undefined
    const editingPart = activeEditEntityId
      ? currentParts.find((part) => part.id === activeEditEntityId || (editAssemblyId && (part.assemblyIds ?? (part.assemblyId ? [part.assemblyId] : [])).includes(editAssemblyId)))
      : undefined
    const editingCustomId = activeEditEntityId?.startsWith('custom:')
      ? activeEditEntityId.slice('custom:'.length)
      : editingPart?.kind === 'custom'
        ? editingPart.partId
        : undefined
    if (activeEditEntityId && !editingCustomId) {
      notifyEditor('当前处于实体编辑模式 · 未找到可修改的普通体素实体')
      return
    }
  ...
  const removeVoxels = (voxels: Voxel[]) => {
    ...
    voxels.forEach((voxel) => {
      ...
      // `partSceneOffset` is used by assembly children and by imported/model
      // parts.  The hit is in effective scene coordinates, so both offsets
      // must be removed before writing the canonical custom voxel back.
      const offset = scenePartGridOffset(part)
    ...
    // Keep the in-flight stroke ownership index in sync with erasures. Without
    // this, a cell erased from an imported/template-derived custom entity can
    // remain marked as occupied until pointer-up, so the next draw is rejected
    // or appears to edit an unrelated owner.
    const transaction = voxelStrokeTransactionRef.current
    if (transaction) {
      voxels.forEach((voxel) => {
        const key = sceneVoxelKey(voxel)
        const owners = transaction.ownerIdsBySceneKey.get(key)
        if (!owners) return
        const entityId = voxelEntityId(voxel)
        if (entityId) owners.delete(`custom:${entityId}`)
        else owners.clear()
        if (!owners.size) {
          transaction.ownerIdsBySceneKey.delete(key)
          transaction.occupiedSceneKeys.delete(key)
        }
      })
    }
    ...
    const activeEditEntityId = voxelStrokeEditEntityRef.current ?? editEntityId
    if (activeEditEntityId?.startsWith('assembly:')) {
      const assemblyId = activeEditEntityId.slice('assembly:'.length)
      ...
      if (assemblyStillExists && !latestParts.some((part) => part.id === selectedId)) {
        setSelectedId(activeEditEntityId)
        setCheckedTreePartIds([activeEditEntityId])
      }
    }
  ...
  const applyVoxelBatch = (voxels: Voxel[], operation: DrawOperation) => {
    ...
    const currentParts = voxelStrokePartsRef.current ?? sceneEntityParts(currentProject)
    const transaction = voxelStrokeTransactionRef.current
    const activeStrokeEditEntityId = transaction
      ? (voxelStrokeEditEntityRef.current ?? editEntityId)
      : editEntityId
    // Resolve ownership in effective scene coordinates. The occupancy index
    // is the fast path; the stroke map and exact custom-part lookup cover the
    // short window after a model/template has been materialized or edited,
    // before the next derived index snapshot is published.
    const ownerIdsAtSceneVoxel = (voxel: Pick<Voxel, 'x' | 'y' | 'z'>) => {
      const key = sceneVoxelKey(voxel)
      // Prefer the current custom-part snapshot over the occupancy index. The
      // latter can briefly retain an old asset owner while a materialized
      // model/template is being published; using that stale owner here would
      // make a valid edit look like a collision.
      const owners = new Set<string>()
      transaction?.ownerIdsBySceneKey.get(key)?.forEach((ownerId) => owners.add(ownerId))
      if (!owners.size) {
        currentParts.forEach((part) => {
          if (scenePartVoxelAtCoordinate(part, voxel.x, voxel.y, voxel.z)) owners.add(part.id)
        })
      }
      if (!owners.size) {
        sceneOccupancyRef.current?.queryProjectVoxel(voxel).ownerIds.forEach((ownerId) => owners.add(ownerId))
      }
      return [...owners]
    }
    if (operation === 'add' && tool !== 'brush') {
      ...
      const assemblyId = activeStrokeEditEntityId?.startsWith('assembly:') ? activeStrokeEditEntityId.slice('assembly:'.length) : null
      const excluded = activeStrokeEditEntityId
        ? (voxelStrokePartsRef.current ?? sceneEntityParts(projectRef.current))
          .filter((part) => part.id === activeStrokeEditEntityId || Boolean(assemblyId && (part.assemblyIds ?? (part.assemblyId ? [part.assemblyId] : [])).includes(assemblyId)))
          .map((part) => part.id)
        : []
    ...
    if (operation === 'add') {
      ...
      // ownership ambiguous.
      const createsNewShapeEntity = tool !== 'brush' && !activeStrokeEditEntityId && operation === 'add'
      const activeEditEntityId = activeStrokeEditEntityId
        ?? (createsNewShapeEntity ? null : (voxelStrokeEntityRef.current ? `custom:${voxelStrokeEntityRef.current}` : null))
      const editAssemblyId = activeEditEntityId?.startsWith('assembly:') ? activeEditEntityId.slice('assembly:'.length) : undefined
      // Every placed/imported object is materialized as customVoxels before it
      // reaches this path. Keep one coordinate and mutation path for hand
      // drawn entities, model imports, and template placements.
      {
        ...
        const insertable = addTargets.filter((voxel) => {
          ...
          // A freehand brush always adds a neighbouring cell. It must never
          // replace a cell already owned by the current entity; that was the
          // subtle difference between editing a hand-drawn part and editing a
          // converted model. Shape/extrude tools may intentionally overwrite
          // the current edit target, but the brush cannot.
          return owners.length === 0 || (tool !== 'brush' && owners.every((ownerId) => excluded.includes(ownerId)))
        })
        ...
        if (insertionPlan.length) updateProject((draft) => {
          ...
          insertionPlan.forEach(({ voxel, entityId: targetEntityId }) => {
            const key = sceneVoxelKey(voxel)
            const owners = transaction?.ownerIdsBySceneKey.get(key) ?? new Set<string>()
            owners.add(`custom:${targetEntityId}`)
            transaction?.ownerIdsBySceneKey.set(key, owners)
            transaction?.occupiedSceneKeys.add(key)
          })
          if (editAssemblyId) {
    ...
    if (operation === 'subtract') {
      ...
      targets.forEach((target) => {
        const ownerIds = ownerIdsAtSceneVoxel(target)
        ownerIds.forEach((ownerId) => {
          const part = partsById.get(ownerId)
          if (!part || (activeStrokeEditEntityId && !partBelongsToEditTarget(part, activeStrokeEditEntityId))) return
          if (part.kind === 'custom') {
            markVoxelStrokeOwners([part.id])
    ...
    if (!activeStrokeEditEntityId) {
      notifyEditor('改色需要先进入实体编辑模式')
      return
    }
    const parts = voxelStrokePartsRef.current ?? sceneEntityParts(projectRef.current)
    updateProject((draft) => {
      const currentCustomIds = new Set(parts.filter((part) => part.kind === 'custom' && partBelongsToEditTarget(part, activeStrokeEditEntityId)).map((part) => part.partId))
  ...
  const raycastSceneVoxel = (origin: { x: number; y: number; z: number }, direction: { x: number; y: number; z: number }): SceneVoxelRayHit | null => {
    ...
    const runtimeRay = sceneWorldRayToRuntime(origin, direction, VOXEL_WORLD_SIZE)
    const hit = raycastVoxelDda(
      runtimeRay.origin,
      runtimeRay.direction,
  ...
  const commitSceneDiscreteTransform = (parts: SceneEntityPart[], mode: 'mirror' | 'rotate', axis: SceneTransformAxis, degrees: 90 | 180 | 270 = 90) => {
    ...
    const selection = resolveDiscreteTransformSelection(sourceProject, parts)
    // Use the same content-based history boundary as every other edit. A
    // transform may reuse unrelated project arrays, but its changed voxel
    // coordinates must always produce a distinct snapshot.
    const recorded = recordHistoryBeforeChange(sourceProject, nextProject)
...
const SceneLibraryPreview = React.memo(function SceneLibraryPreview({ cacheKey, voxels, maxVoxels, exteriorOnly = false }: { cacheKey: string; voxels: ScenePreviewInputVoxel[]; maxVoxels: number; exteriorOnly?: boolean }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const faceNormal = (orientation: number): readonly [number, number, number] => {
        if (orientation === 0) return [0, 1, 0]
        if (orientation === 3) return [0, -1, 0]
        if (orientation === 1) return [1, 0, 0]
        if (orientation === 4) return [-1, 0, 0]
        if (orientation === 2) return [0, 0, 1]
        return [0, 0, -1]
      }
      ...
      payload.faces.forEach((face) => {
        ...
        context.closePath()
        const baseRgb: [number, number, number] = [(face.color >> 16) & 0xff, (face.color >> 8) & 0xff, face.color & 0xff]
        const shaded = studioShadeRgb(baseRgb, faceNormal(face.orientation))
        context.fillStyle = `rgb(${shaded.join(', ')})`
        context.fill()
      })
    }
    draw()
...
function VariantSurfaceMiniPreview({ voxels, asset, colorOverride, voxelColors = {}, materialColors = {}, maxPreviewVoxels, exteriorOnly = false }: { voxels: Voxel[]; asset?: VoxelAsset; colorOverride?: string; voxelColors?: Record<string, string>; materialColors?: Record<string, string>; maxPreviewVoxels?: number; exteriorOnly?: boolean }) {
  ...
  const triangles = Array.from({ length: surface.indices.length / 3 }, (_, triangleIndex) => {
    ...
    const shaded = studioShadeRgb([color[0], color[1], color[2]], [normal[0], normal[1], normal[2]], ao)
    const fill = `rgb(${shaded.join(', ')})`
...
const SynchronousVoxelMiniPreview = React.memo(function SynchronousVoxelMiniPreview({ voxels, asset, colorOverride, voxelColors = {}, materialColors = {}, maxPreviewVoxels, exteriorOnly = false }: { voxels: Voxel[]; asset?: VoxelAsset; colorOverride?: string; voxelColors?: Record<string, string>; materialColors?: Record<string, string>; maxPreviewVoxels?: number; exteriorOnly?: boolean }) {
  ...
  const faceNormal = (orientation: typeof faceRects[number]['orientation']): readonly [number, number, number] => {
    if (orientation === 'top') return [0, 1, 0]
    if (orientation === 'bottom') return [0, -1, 0]
    if (orientation === 'x') return [1, 0, 0]
    if (orientation === 'x-negative') return [-1, 0, 0]
    if (orientation === 'z') return [0, 0, 1]
    return [0, 0, -1]
  }
  ...
  faceRects.forEach((face) => {
    const fill = studioShadeHex(face.color, faceNormal(face.orientation))
...
function configureHslPreviewMaterial(material: THREE.MeshStandardMaterial, cacheKey: string, enableVoxelAo = false) {
  ...
  material.onBeforeCompile = (shader) => {
    ...
    const colorFragment = `#include <color_fragment>
      ...
      ${enableVoxelAo ? `diffuseColor.rgb *= mix(${STUDIO_RENDER_SETTINGS.aoFloor.toFixed(2)}, 1.0, clamp(vMoceAo, 0.0, 1.0));` : ''}`
...
function createVoxelSurfaceRenderMesh(
  ...
  geometry.computeBoundingSphere()
  const material = createStudioMaterial({ vertexColors: true })
...
function VoxelViewport({ project, authoritativeProjectRef, liveEditEntityIdRef, sceneParts, occupancyIndex, assetTransformCache, selectedId, selectionRefreshKey, selectedPartIds, checkedPartIds, lockedPartIds, editEntityId, colorPreview, geometryPreview, geometryApplying, historyResetKey, tool, toolboxOpen, drawingPlane, drawOperation, brushSize, voxelBrushShape, activeMaterial, materials, dragAxis, placementAsset, copyPreview, transformPreview, transformPreviewRevision, viewMode, showGrid, showBoundary, zoomLevel, onZoomChange, onCameraApiChange, onInteractionChange, onRaycastVoxel, onSyncSceneOccupancyTransforms, onSelect, onSelectMultiple, onCancelPendingEntityOperation, onSelectMaterial, onReplaceMaterial, onAddVoxel, onRemoveVoxel, onRemoveVoxels, onEditInstanceVoxel, onEditInstanceVoxels, onApplyVoxelBatch, onPreviewScenePartsMove, onCommitScenePartsMove, onCancelScenePartsMove, onPreviewPlacement, onPlaceAsset, onNotice, onExitEditMode, onEnterEditMode, onRename, onBatchOperation, children }: { project: ProjectState; authoritativeProjectRef: React.MutableRefObject<ProjectState>; liveEditEntityIdRef: React.MutableRefObject<string | null>; sceneParts: SceneEntityPart[]; occupancyIndex: SceneOccupancyIndex | null; assetTransformCache: AssetTransformCache; selectedId: string; selectionRefreshKey: number; selectedPartIds: string[]; checkedPartIds: string[]; lockedPartIds: Set<string>; editEntityId: string | null; colorPreview: ColorPreviewState | null; geometryPreview: GeometryPreviewState | null; geometryApplying: boolean; historyResetKey: number; tool: Tool; toolboxOpen: boolean; drawingPlane: DrawingPlane; drawOperation: DrawOperation; brushSize: number; voxelBrushShape: VoxelShape; activeMaterial: string; materials: Material[]; dragAxis: 'horizontal' | 'vertical'; placementAsset: VoxelAsset | null; copyPreview: CopyPreviewState | null; transformPreview: DiscreteTransformPreviewState | null; transformPreviewRevision: number; viewMode: '正交' | '透视'; showGrid: boolean; showBoundary: boolean; zoomLevel: number; onZoomChange: (value: number) => void; onCameraApiChange: (api: CameraControlApi | null) => void; onInteractionChange: (active: boolean) => void; onRaycastVoxel: (origin: { x: number; y: number; z: number }, direction: { x: number; y: number; z: number }) => SceneVoxelRayHit | null; onSyncSceneOccupancyTransforms: () => void; onSelect: (id: string) => void; onSelectMultiple: (partIds: string[], additive?: boolean) => void; onCancelPendingEntityOperation: () => void; onSelectMaterial: (id: string) => void; onReplaceMaterial: (id: string, color: string) => void; onAddVoxel: (voxel: Voxel) => void; onRemoveVoxel: (voxel: Voxel) => void; onRemoveVoxels: (voxels: Voxel[]) => void; onEditInstanceVoxel: (instanceId: string, voxel: Voxel, mode: VoxelOverride['mode']) => void; onEditInstanceVoxels: (instanceId: string, voxels: Voxel[], mode: VoxelOverride['mode']) => void; onApplyVoxelBatch: (voxels: Voxel[], operation: DrawOperation) => void; onPreviewScenePartsMove: (parts: SceneEntityPart[], deltaX: number, deltaY: number, deltaZ: number) => GridMoveResult; onCommitScenePartsMove: (parts: SceneEntityPart[], deltaX: number, deltaY: number, deltaZ: number) => GridMoveResult; onCancelScenePartsMove: () => void; onPreviewPlacement: (assetId: string, x: number, z: number) => PlacementPreview | null; onPlaceAsset: (assetId: string, x: number, z: number) => void; onNotice: (message: string) => void; onExitEditMode: () => void; onEnterEditMode: (entityId: string) => void; onRename: (targetId: string, assemblyId?: string) => void; onBatchOperation: (partIds: string[], operation: 'delete' | 'lock' | 'assemble') => void; children?: React.ReactNode }) {
  ...
  // Entering edit mode is also a pivot reset, even when the selected part IDs
  // did not change. After that reset the live OrbitControls target is allowed
  // to move with the user's pan gesture.
  const orbitSelectionKey = `${editEntityId ?? ''}::${selectedPartIds.join('|')}`
  ...
  const materialMap = useMemo(() => new Map(project.materials.map((material) => [material.id, createStudioMaterial({ color: material.color })])), [project.materials])
  ...
  useEffect(() => {
    ...
    if (toolPreviewMesh) {
      ...
      geometry.computeBoundingSphere()
      const material = createStudioMaterial({ color: subtractPreview ? '#e06b5b' : '#ffffff', vertexColors: !subtractPreview, transparent: true, opacity: 0.34, depthWrite: false })
  ...
  useEffect(() => {
    ...
    byColor.forEach((voxels, color) => {
      const mesh = new THREE.InstancedMesh(sharedVoxelBoxGeometry, createStudioMaterial({ color, transparent: true, opacity: previewState?.valid ? 0.34 : 0.18, depthWrite: false }), voxels.length)
      const matrix = new THREE.Matrix4()
  ...
  useEffect(() => {
    ...
    renderer.toneMappingExposure = STUDIO_RENDER_SETTINGS.exposure
    ...
    const floor = new THREE.Mesh(new THREE.PlaneGeometry(initialBounds.x * VOXEL_WORLD_SIZE, initialBounds.y * VOXEL_WORLD_SIZE), createStudioMaterial({ color: '#11181b', side: THREE.DoubleSide, depthWrite: false }))
    floor.position.z = 0
    ...
    // OrbitControls.target is the actual center of the current view. A pan
    // changes this target, so the following rotation must orbit around the
    // user's current view center instead of jumping back to the selection.
    // Keep the selection pivot as the reset value, but promote every real
    // target change (mouse/touch pan, view command, or selection sync) to the
    // live pivot.
    const lastOrbitTarget = controls.target.clone()
    const handleControlsChange = () => {
      if (controls.target.distanceToSquared(lastOrbitTarget) < 0.00000001) return
      lastOrbitTarget.copy(controls.target)
      orbitPivotRef.current = controls.target.clone()
    }
  ...
  const applyCameraView = (view: CameraView, requestedZoom = cameraZoomLevelRef.current) => {
    ...
    // View commands change orientation and framing, but they are not a new
    // selection. Preserve the live pivot established by the current camera
    // target, including a target moved by pan. The selection effect is the
    // only place that intentionally resets this back to entity center.
    const target = orbitPivotRef.current?.clone()
      ?? selectedOrbitCenterRef.current?.clone()
      ?? sceneViewTarget(sceneBounds)
  ...
  const rotateCameraByInput = (deltaX: number, deltaY: number) => {
    const controls = controlsRef.current
    if (!controls) return
    // Do not re-anchor here. On initial selection/edit entry the selection
    // effect sets the pivot once. After a pan, OrbitControls.target is the
    // user's current view center and must remain the rotation center.
  ...
  useEffect(() => {
    // Change the pivot only when the selection/edit identity changes. A voxel
    // edit changes selectedOrbitCenter because the AABB changes, but it must
    // not move the camera or the entity on screen. Clearing the selection
    // deliberately preserves the current camera target so the next view
    // operation does not create a second, hidden camera jump.
    if ((selectedPartIds.length > 0 || Boolean(editEntityId)) && !selectedOrbitCenter) return
    ...
    if (selectedOrbitCenter) {
      orbitPivotRef.current = selectedOrbitCenter.clone()
      syncOrbitTarget(selectedOrbitCenter)
    } else if (controlsRef.current) {
      orbitPivotRef.current = controlsRef.current.target.clone()
    } else {
      orbitPivotRef.current = null
    }
  }, [editEntityId, orbitSelectionKey, selectedOrbitCenter, selectedPartIds.length])
  ...
  const extrudeParts = () => {
    const activeEditEntityId = liveEditEntityIdRef.current ?? editEntityId
    const editAssemblyId = activeEditEntityId?.startsWith('assembly:') ? activeEditEntityId.slice('assembly:'.length) : null
    const parts = sceneEntityParts(authoritativeProjectRef.current)
    return parts.filter((part) => !activeEditEntityId || part.id === activeEditEntityId || Boolean(editAssemblyId && (part.assemblyIds ?? (part.assemblyId ? [part.assemblyId] : [])).includes(editAssemblyId)))
  }
  ...
  const handleEntityDoubleClick = (event: { clientX: number; clientY: number }) => {
    ...
    const context = getPointerContext(event)
    const hitPart = resolvePointerHitPart(context?.voxelHit, sceneEntityParts(authoritativeProjectRef.current))
    if (!hitPart) return
  ...
  const partBelongsToEditEntity = (part: SceneEntityPart | undefined) => {
    // Asset/template instances are materialized synchronously at the start of
    // a drawing gesture. React state may still contain the old asset:* ID for
    // this event, so all edit permission checks must use the live remapped ID
    // before falling back to the rendered prop.
    const activeEditEntityId = liveEditEntityIdRef.current ?? editEntityId
    if (!part || !activeEditEntityId) return Boolean(part)
    if (part.id === activeEditEntityId) return true
    if (!activeEditEntityId.startsWith('assembly:')) return false
    const assemblyId = activeEditEntityId.slice('assembly:'.length)
    return (part.assemblyIds ?? (part.assemblyId ? [part.assemblyId] : [])).includes(assemblyId)
  }

  // ownerIds is only a raycast/occupancy hint and can be stale for one frame
  // after imported or asset-backed data is materialized. Editing resolves the
  // authoritative scene-space coordinate first, so every entity uses the same
  // custom-voxel batch path as a hand-drawn entity.
  const resolvePointerHitPart = (hit: SceneVoxelRayHit | null | undefined, parts: SceneEntityPart[]) => {
    if (!hit) return undefined
    const exactParts = parts.filter((part) => Boolean(scenePartVoxelAtCoordinate(part, hit.voxel.x, hit.voxel.y, hit.voxel.z)))
    if (!exactParts.length) return undefined
    const hintedIds = new Set(hit.ownerIds)
    // The raycast owner hint is authoritative when it names one of the exact
    // scene-space parts. Do not prefer `custom` merely because it is the
    // storage kind: that preference can select a stale hand-drawn fragment
    // over the model part that is actually under the cursor.
    return exactParts.find((part) => hintedIds.has(part.id)) ?? exactParts[0]
  }
  ...
  const applyStrokeAdd = (voxel: Voxel, key = `scene:${strokeVoxelKey(voxel)}`) => {
    ...
    // Keep the compatibility pointer path on the same batch/edit pipeline as
    // continuous drawing and shape tools. Calling the legacy single-voxel
    // callback here made imported/template-derived entities take a different
    // ownership and collision path from hand-drawn entities.
    onApplyVoxelBatch([voxel], 'add')
  }
  const applyStrokeRemove = (voxel: Voxel, key = `scene:${strokeVoxelKey(voxel)}`) => {
    const operationKey = `remove:${key}`
    if (editStrokeVisitedRef.current.has(operationKey)) return
    editStrokeVisitedRef.current.add(operationKey)
    onApplyVoxelBatch([voxel], 'subtract')
  }
  const applyEditAtPointer = (event: { clientX: number; clientY: number }, options: { allowFloorFallback?: boolean } = {}) => {
    ...
    const currentSceneParts = sceneEntityParts(currentProject)
    const hitPart = resolvePointerHitPart(voxelHit, currentSceneParts)
    ...
    if (voxelHit && hitPart) {
      ...
      // Imported models and asset-library placements are materialized into
      // scene customVoxels. Do not route them through the old instance-local
      // override path: it uses a different coordinate origin and is the
      // reason drawing can replace an internal voxel while erase misses.
    ...
    if (tool === 'brush') {
      const floorOwners = new Set(occupancyIndex?.queryProjectVoxel(sceneVoxel).ownerIds ?? [])
      currentSceneParts.forEach((part) => {
        if (scenePartVoxelAtCoordinate(part, sceneVoxel.x, sceneVoxel.y, sceneVoxel.z)) floorOwners.add(part.id)
      })
      if (floorOwners.size) onNotice('目标网格已有体素 · 请点击体素表面添加')
      else applyStrokeAdd(sceneVoxel)
    } else {
      const bounds = sceneBoundsForProject(currentProject)
      let highest: Voxel | undefined
      currentSceneParts.forEach((part) => {
        scenePartVoxels(part).forEach((voxel) => {
          if (voxel.x !== x || voxel.z !== z || voxel.y < 0 || voxel.y > bounds.z - 1) return
          if (!highest || voxel.y > highest.y) highest = voxel
        })
      })
  ...
  const handleEditPointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (tool === 'select') {
      ...
      const currentProject = authoritativeProjectRef.current
      const currentSceneParts = sceneEntityParts(currentProject)
      const hitPart = resolvePointerHitPart(context?.voxelHit, currentSceneParts)
      if (hitPart) {
        const explicitMultiSelection = selectedPartIds.length > 1 && (checkedPartIds.length > 1 || (checkedPartIds.length === 1 && !checkedPartIds[0].startsWith('assembly:')))
    ...
    // Materialize legacy asset/template instances before calculating the
    // pointer hit.  Previously this happened only in the later interaction
    // callback, after pointerDrawingPoint/pointerShapeGroundPoint had already
    // queried the old instance occupancy.  The first stroke on an imported
    // model could therefore use the old transform and place a cell inside or
    // on top of the model instead of at its adjacent face.
    setViewportInteraction(true)
...
function buildCustomComponentGroup(component: Voxel[], entityId: string, materialMap: Map<string, THREE.MeshStandardMaterial>, componentColor?: string, forceCellRender = false) {
  ...
  if (deferCellBuild) {
    ...
    for (let start = 0; start < component.length; start += cellChunkSize) {
      ...
      const meshMaterial = createStudioMaterial({ color: '#ffffff', vertexColors: true })
  ...
  batches.forEach(({ color, voxels }) => {
    const meshMaterial = createStudioMaterial({ color })
...
function buildAssetGroup(asset: VoxelAsset, materialMap: Map<string, THREE.MeshStandardMaterial>, overrides: VoxelOverride[] = [], partOffsets: SceneInstance['partOffsets'] = {}, rotation = 0, colorOverride?: string, mirror: SceneInstance['mirror'] = undefined, rotationX = 0, rotationY = 0, rotationZ = 0, allowGreedyMesh = false, greedyCacheKey?: string, rotationPivot: SceneInstance['rotationPivot'] = undefined) {
  ...
  if (!resolvedComponents.length) {
    const placeholder = new THREE.Mesh(new THREE.BoxGeometry(asset.width * scale, asset.depth * scale, asset.height * scale), createStudioMaterial({ color: asset.color }))
  ...
  for (const { partId, voxels: component } of resolvedComponents) {
    ...
    if (component.some((voxel) => voxelShape(voxel) !== 'cube')) {
      ...
      cubeBatches.forEach(({ color, voxels }) => {
        const mesh = new THREE.InstancedMesh(sharedVoxelBoxGeometry, createStudioMaterial({ color }), voxels.length)
      ...
      variantBatches.forEach(({ color, voxel, voxels, includeMountingFace }) => {
        const mesh = new THREE.InstancedMesh(sharedVariantThreeGeometry(voxel, includeMountingFace), createStudioMaterial({ color }), voxels.length)
        voxels.forEach((cell, index) => {
          setLocalMatrix(cell, matrix)
          mesh.setMatrixAt(index, matrix)
        })
    ...
    batches.forEach(({ color, voxels }) => {
      const material = createStudioMaterial({ color })
